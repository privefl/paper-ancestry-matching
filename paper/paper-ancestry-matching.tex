...
\abstract{
	...
	The proposed method is particularly important for methods that needs to be applied in homogeneous samples.
...
We do not provide a method to define admixture coefficients nor infer local ancestry, which are different problems for which there are several existing methods \cite[]{alexander2009fast,frichot2014fast,raj2014faststructure,padhukasahasram2014inferring}.
However, inferring global ancestry in non-admixed individuals is still a very important problem since there are methods that specifically need to be applied in samples of ancestry homogeneous.
...
We report $F_{ST}$ between the 26 1000G populations in tables \ref{tab:fst-AFR}-\ref{tab:fst-SAS}.
...
In contrast, the two other Bhattacharyya and shortest distances do not provide as satisfactory results (Figures \ref{fig:heatmap1} and \ref{fig:heatmap3}). For example, African Caribbeans in Barbados (ACB) and Americans of African Ancestry in SW USA (ASW) are close to Northern European populations (FIN, CEU and GBR) when using the Bhattacharyya distance.
...
When an individual is not close enough to any of the 26 1000G populations, we leave its ancestry inference as unknown, otherwise we assign this individual to the closest population.
...
However, when comparing the location of these mismatched individuals to the rest of individuals on the PCA space computed on the UK Biobank \cite[]{bycroft2017genome}, it seems more probable that our ancestry estimate is exact while the self-reported ancestry is wrong (Figure \ref{fig:mismatch}).
...
Finally, we show 3 possible ways how to use our ancestry inference method for grouping genetically homogeneous individuals.
The first approach is to simply match individuals that are closed enough to one or more of the 1000G populations, as described in this paper.
...
Another solution, which does not require projecting individuals to the 1000G, but does require computing PC scores within the dataset instead, would consist in using reported ancestries and internal PC scores.
...
This enables e.g.\ to capture a larger set of individuals who are close enough to British people (e.g.\ Irish people), while discarding individuals whose genetic ancestry is not really ``British'' as self-reported. 
A third solution, useful when we know that the data is composed of a predominant ancestry is to keep individuals who are close enough to the overall center of all individuals (Figure \ref{fig:hist3}).
...
\begin{figure}[h]
	...
	\caption{Locations of the 1000G populations, accessed from \url{https://www.internationalgenome.org/data-portal/population} and represented using R package leaflet. 
	...
	Also note that Utah Residents with Northern and Western European Ancestry, Americans of African Ancestry in SW USA, African Caribbeans in Barbados and Mexican Ancestry from Los Angeles USA are not located at their ancestral location. \label{fig:map}}
\end{figure}
